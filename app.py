...
def save_data(df):
    ensure_data_file()
    backup = DATA_DIR / f"alllinks_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    if DATA_FILE.exists():
        shutil.copy2(DATA_FILE, backup)
    df.to_excel(DATA_FILE, sheet_name=SHEET_NAME, index=False)
    # Keep only last 5 backups
    backups = sorted(DATA_DIR.glob("alllinks_backup_*.xlsx"))
...
def render_table(df):
    ...
    for idx, (_, row) in enumerate(df.iterrows()):
        ...
        html += f"""<tr>
            ...
            <td class="name-cell">{row.get('Application Name','—')}</td>
            <td><span style="font-size:12px;">{type_icon} {row.get('Link Type','')}</span></td>
            <td class="desc-cell" title="{row.get('Description','')}">{row.get('Description','—')}</td>
            ...
            <td class="meta-cell">{mod_date if mod_date and mod_date != 'nan' else '—'}</td>
...
def main():
    ...
    with st.sidebar:
        ...
        # Export Excel
        if total > 0:
            export_data = df.to_excel.__self__  # just for the button
            import io
            buf = io.BytesIO()
        ...
        # Notify All
        if total > 0:
            hub_url = st.text_input("Hub URL (for emails)", value="http://your-server:8501", key="hub_url")
            ...
            st.markdown(f'<a href="mailto:?subject={subj}&body={body}" style="display:inline-block;padding:8px 16px;background:#3B82F6;color:white;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600;">📧 Notify All Team</a>', unsafe_allow_html=True)
    ...
    if st.session_state.show_add_form or st.session_state.edit_idx is not None:
        ...
        with st.expander(title, expanded=True):
            edit_row = df.iloc[st.session_state.edit_idx] if editing else {}

            c1, c2 = st.columns(2)
            with c1:
                f_category = st.text_input("Category", value=str(edit_row.get("Category", "")), key="f_cat_input")
                f_link_type = st.selectbox("Link Type", ["Power BI", "SharePoint", "Excel", "Web Page", "Application", "Document", "PPT / Deck", "Ops Resource", "Other"],
                                           index=0, key="f_type_input")
                f_owner = st.text_input("Owner", value=str(edit_row.get("Owner", "")), key="f_owner")
                f_update = st.selectbox("Update Available", ["No", "Yes"], key="f_update")
                f_dept = st.text_input("Department", value=str(edit_row.get("Department", "")), key="f_dept")
            with c2:
                f_name = st.text_input("Application Name *", value=str(edit_row.get("Application Name", "")), key="f_name")
                f_url = st.text_input("URL", value=str(edit_row.get("URL", "")), key="f_url")
                f_status_input = st.selectbox("Status", ["Active", "In Progress", "Pending", "Retired", "Issue"], key="f_status_input")
                f_version = st.text_input("Version", value=str(edit_row.get("Version", "")), key="f_version")
                f_priority = st.selectbox("Priority", ["Medium", "High", "Low"], key="f_priority")

            f_desc = st.text_area("Description", value=str(edit_row.get("Description", "")), key="f_desc", height=68)
            f_remarks = st.text_area("Remarks", value=str(edit_row.get("Remarks", "")), key="f_remarks", height=68)
    ...
    st.markdown(
        ...
        f'CENTRALIZED ACCESS HUB · Data file: {DATA_FILE.absolute()} · '
